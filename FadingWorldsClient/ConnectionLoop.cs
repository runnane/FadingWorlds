    ...
    public class ConnectionLoop : IDisposable
    {
        ...
        public void StartConnect(string szRemoteHostName, int iRemotePort, string username, string password)
        {
            try
            {
                ...
                IsConnected = true;
                while (!IsShuttingDown && !IsDisconnecting && _client.Connected && TheLoader.State != GameState.StoppingGameWindow)
                {
                    // Wait for command
                    ...
                    //    break;
                    //}
                    if (payload.Type == PayloadType.Unset) continue;
